...
type ToJoinType<OldType, NewType extends JoinType> = Extract<OldType, "left-side-of-right-join"> extends never
  ? NewType
  : OldType;
...
type AddRightJoin<Columns, JoinTable> = {
  ...
  >
    ? Extract<GetTableName<JoinTable>, TableName> extends never
      ? Column<Name, TableName, DataType, IsNotNull, HasDefault, ToJoinType<JoinType, "left-side-of-right-join">>
      : Columns[K]
    : never;
};
...
type GetColumns<From extends Table<any, any> | FromItem<any>> = From extends Table<any, infer Columns>
  ? Columns
...
export class SelectQuery<Columns extends { [column: string]: any }, IncludesStar = false> extends Query<Columns> {
  ...
  limit(limit: number | "ALL"): SelectQuery<Columns> {
    if (limit === `ALL`) {
      ...
      return this.newSelectQuery([...this.tokens, new StringToken(`LIMIT`), new ParameterToken(limit)]);
    }
  }

  // [ OFFSET start [ ROW | ROWS ] ]
  offset(start: number): SelectQuery<Columns> {
    return this.newSelectQuery([...this.tokens, new StringToken(`OFFSET`), new ParameterToken(start)]);
  }
  ...
  of(table: Table<any, any>): SelectQuery<Columns> {
    return this.newSelectQuery([...this.tokens, new StringToken(`OF`), new StringToken(table.getName())]);
  }
...
export const makeSelect = (queryExecutor: QueryExecutorFn, initialTokens?: Token[]): SelectFn => <T extends Selectable>(
  ...columns: T[]
) => {
