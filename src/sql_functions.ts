...
export function raw<DataType = unknown, IsNotNull extends boolean = true, Name extends string = never>(
...
export const bitOr = <T extends AnyNumber>(expression: Expression<T, boolean, any>) =>
  new Expression<T, false, "bitOr">([new StringToken(`bit_or`), new GroupToken(expression.toTokens(false))], "bitOr");
...
export const min = <DataType>(expression: Expression<DataType, boolean, any>): Expression<DataType, false, "min"> =>
  new Expression<DataType, false, "min">([new StringToken(`MIN`), new GroupToken(expression.toTokens())], "min");

export const max = <DataType>(expression: Expression<DataType, boolean, any>) =>
  new Expression<DataType, false, "max">([new StringToken(`MAX`), new GroupToken(expression.toTokens())], "max");
...
export const exists = (expression: Expression<any, any, any> | Query<any>): DefaultExpression<boolean> =>
  new DefaultExpression([new StringToken(`EXISTS`), new GroupToken(expression.toTokens())]);

export const andNotExists = (expression: Expression<any, any, any> | Query<any>): DefaultExpression<boolean> =>
  new DefaultExpression([new StringToken(`AND NOT EXISTS`), new GroupToken(expression.toTokens())]);

export const andExists = (expression: Expression<any, any, any> | Query<any>): DefaultExpression<boolean> =>
  new DefaultExpression([new StringToken(`AND EXISTS`), new GroupToken(expression.toTokens())]);

export const notExists = (expression: Expression<any, any, any> | Query<any>): DefaultExpression<boolean> =>
  new DefaultExpression([new StringToken(`NOT EXISTS`), new GroupToken(expression.toTokens())]);

export const coalesce = <DataType>(...expressions: (Expression<DataType, boolean, any> | DataType)[]) => {
